...
- 配色：macOS 原生偏好设置面板扁平风格，蓝色系强调色，实心纯色背景（无半透明/渐变/毛玻璃），支持深色/浅色模式自动跟随系统，统一使用 theme.py 模块
- 主窗口使用纯色背景（浅灰 #F5F5F7 / 深灰 #1C1C1E），Tab 栏使用图标+文字风格
- 卡片和面板使用实心白色背景 + apply_shadow 轻柔阴影

## Prompt 管理面板 (PromptPanel)

### 布局
- 顶部工具栏：搜索框（300ms 防抖）+ 标签筛选下拉框 + 同步按钮 + 新建按钮
- 中间：QScrollArea 内使用 FlowLayout 网格布局展示 PromptCard 卡片（每行 2-3 张，根据窗口宽度自适应换行）
- 空状态提示

### 功能
- 搜索和标签筛选可以同时生效（先按标签筛选再匹配关键词）
- 标签筛选：下拉框含 "全部标签" + 所有已有标签，点击卡片上的标签也可直接筛选
- 新建/编辑通过 PromptEditDialog 弹窗
- 删除有确认对话框（软删除）
- 复制将内容写入系统剪贴板
- 同步按钮：发射 sync_requested 信号，由 app 连接到 SyncEngine.sync_all 触发与所有在线设备的双向同步
...
### DeviceCard
- 使用 theme.card_style() + apply_shadow 轻柔阴影
...
- 不置顶（去掉 WindowStaysOnTopHint），避免遮挡用户授权时打开的"系统设置"窗口
...
### 布局（参考 macOS Mos 应用更新提示风格）
- QVBoxLayout（spacing=16, margins=24,24,24,20），最小宽度 560
- 顶部 header：左侧 64x64 应用图标 + 右侧两行文字
  - 主标题："新版本的 Claude Partner 已经发布"（17px 粗体）
  - 副标题："Claude Partner X.Y.Z 可供下载，您现在的版本是 A.B.C。要现在下载吗？（安装包 X.X MB）"
- 更新内容（可选）：QTextEdit 渲染 Markdown（setMarkdown），最小高度 200，
  卡片化样式（细边框 + 圆角 + BG_PRIMARY 背景 + 12/16 内边距）
- 进度区域（初始隐藏）：QProgressBar + QLabel 状态文字
- 按钮行：次要按钮左对齐（跳过这个版本/取消/稍后再说）+ 弹簧 + 主按钮右对齐（安装更新/下载中…/安装并重启/重试）
