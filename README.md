...
| 系统 | 文件名 | 说明 |
|------|--------|------|
| Ubuntu / Linux (x86_64) | `ClaudePartner-ubuntu-x86_64` | 直接运行的可执行文件 |
| macOS (即将支持) | - | 开发中 |
| Windows (即将支持) | - | 开发中 |
...
1. **下载文件**
   ...
   从 [Releases](https://github.com/mmletgo/claude-partner/releases) 页面下载 `ClaudePartner-ubuntu-x86_64`

2. **赋予执行权限**

   打开终端，进入下载目录，执行：
   ```bash
   chmod +x ClaudePartner-ubuntu-x86_64
   ```

3. **运行程序**
   ```bash
   ./ClaudePartner-ubuntu-x86_64
   ```

   > 首次运行时系统可能会提示安全警告，点击允许即可。

4. **（可选）创建桌面快捷方式**

   创建文件 `~/.local/share/applications/claude-partner.desktop`，内容如下：
   ```ini
   [Desktop Entry]
   Name=Claude Partner
   Exec=/你的路径/ClaudePartner-ubuntu-x86_64
   Icon=/你的路径/icon.png
   Type=Application
   Categories=Utility;
   ```
   将 `/你的路径/` 替换为你实际存放文件的路径。
...
- Ubuntu 用户确保已安装必要的系统库：
  ...
  ```
- 确认文件有执行权限：`chmod +x ClaudePartner-ubuntu-x86_64`
