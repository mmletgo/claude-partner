...
- Push 按钮只在当前 worktree 有分支且后端 status 判定存在可用推送目标时启用；本地未发布、没有任何 remote 的项目保持禁用。
...
- Rust `workbench/git.rs` 在 worktree status 中派生 `canPush`，规则与 push 命令的 upstream/origin/唯一 remote 选择保持一致。
...
- Rust 单测覆盖无 remote 时 `canPush=false`、有 origin remote 时 `canPush=true`。
