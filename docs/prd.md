...
- 工作台终端界面可唤出 Prompt 优化浮层，复用同一优化能力生成中英文结果；优化时应以当前项目根目录作为 Claude Code 工作目录，使其可读取项目 CLAUDE.md 上下文；结果可一键把中文优先、英文兜底的文本填入当前运行中的终端，填入只插入文本，不自动追加回车或执行命令
...
- Prompt 优化浮层：终端工具栏提供 Prompt 优化入口，浮层不阻塞终端使用；优化请求绑定当前项目根目录以加载项目 CLAUDE.md 上下文；优化失败时展示诊断信息，填入按钮仅在当前活动 session 为 running 且有优化结果时可用
