...
- 多普通终端：在当前项目根目录中通过 PTY 启动系统 shell（用户自行运行 `claude` 或其他命令），支持多个会话 tab
...
- 会话操作：支持重命名、关闭；关闭/应用退出清理时如果底层进程已自然退出或被系统回收，应视为清理成功，不向用户展示 No such process 类 IO 错误；应用退出时必须清理仍运行的工作台 PTY 子进程，避免后台残留终端进程
