...
**描述**：以项目文件夹为中心管理 Git worktree、本机普通交互式终端、当前工作区文件夹和 Git 提交历史。第一期支持本机目录及已挂载局域网目录；远端 cc-partner 设备上的原生远端项目、远端 PTY 和文件内容预览后续扩展。

**功能点**：
- 工作台布局：项目文件夹列表紧跟全局左侧栏“设置”菜单项下方，作为进入工作台的入口，不再设置独立“工作台”主导航项；主区域依次展示工作台标题、terminal sessions 标识、worktree 管理层、依赖提示槽、当前 worktree 的 window tabs 和终端面板；右侧检查器承载当前 window 状态，并提供当前 worktree 文件夹 / Git 提交历史 tab，窄宽下排到首屏终端之后
...
- Worktree 管理：每个项目默认有一个主 worktree，路径为用户添加的项目根目录；顶部 worktree 管理层用于创建功能 worktree、切换 active worktree、查看 clean/dirty/conflict 状态，并提供 AI commit、push、merge 到主工作区和移除非主 worktree 操作。创建 worktree 时在应用数据目录下执行 `git worktree add -b <branch>`；commit 点击后不弹手写输入框，后端执行 `git add -A`、读取 staged diff/stat，并在 active worktree cwd 下用 Claude Code 项目上下文模式生成 commit message 后 `git commit -m`；push 优先复用当前分支 upstream 执行 `git push`，没有 upstream 时选择 `origin` 或唯一 remote 执行 `git push -u <remote> <branch>`，完全没有 remote 或多 remote 且无 origin/upstream 时返回配置提示；merge 前必须要求源 worktree 和主工作区均 clean 且无冲突；移除 worktree 前必须先关闭该 worktree 下的 terminal window。第一期不做 Git diff 面板、交互式冲突解决和 PR 创建
- Git 提交历史：右侧检查器提供“项目文件夹 / Git 历史”tab；Git 历史按 active worktree 查询最近提交，展示短 hash、提交摘要、作者和相对时间，并提供手动刷新；切换项目/worktree、commit 或 merge 后刷新，空仓库或无提交时显示空态
