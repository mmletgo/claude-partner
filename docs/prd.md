...
- 工作台终端界面可通过工具栏或快捷键唤出 Prompt 优化浮层；浮层只显示一个原始 Prompt 输入框，不显示优化按钮、填入终端按钮、双语结果标题/结果区或关闭按钮；优化时应以当前项目根目录作为 Claude Code 工作目录，使其可读取项目 CLAUDE.md 上下文，并只按设置页选择的中文/英文语种生成一个优化版 Prompt；默认快捷键为轻按 Control 单键，首次触发打开浮层并聚焦原始 Prompt 输入框，再次触发自动优化并填入当前运行中的终端，填入后自动关闭浮层；填入只插入文本，不自动追加回车或执行命令
...
- Prompt 优化浮层：终端工具栏提供 Prompt 优化入口，默认也可轻按 Control 唤起；浮层悬浮在当前终端输入光标附近，只渲染一个原始 Prompt 输入框，每次从关闭态重新打开时清空输入；首次快捷键触发打开并聚焦原始 Prompt 输入框，再次触发自动优化并按设置语言填入当前 running active session，成功填入后自动关闭浮层；优化请求绑定当前项目根目录以加载项目 CLAUDE.md 上下文，但输出仍必须是需求方视角的直接委托式 Prompt，不能把项目文档规则扩展成向用户确认 `docs/` 或写文件的澄清问题；小组件不显示“优化/填入终端/中文优化版/English optimized/关闭”等按钮或双语结果区，且不请求双语优化
