...
- Prompt 优化浮层：终端工具栏提供 Prompt 优化入口，默认也可轻按 Control 唤起；浮层悬浮在当前终端输入光标附近，首次快捷键触发打开并聚焦原始 Prompt 输入框，再次触发自动优化并按设置语言填入当前 running active session；优化请求绑定当前项目根目录以加载项目 CLAUDE.md 上下文；优化失败时展示诊断信息，填入按钮仅在当前活动 session 为 running 且有优化结果时可用
