...
- 工作台终端界面可通过工具栏或快捷键唤出 Prompt 优化浮层，复用同一优化能力生成中英文结果；优化时应以当前项目根目录作为 Claude Code 工作目录，使其可读取项目 CLAUDE.md 上下文；默认快捷键为轻按 Control 单键，首次触发打开浮层并聚焦原始 Prompt 输入框，再次触发自动优化并按设置页选择的中文/英文优化版填入当前运行中的终端；填入只插入文本，不自动追加回车或执行命令
...
**功能点**：
- 工作台布局：项目文件夹列表紧跟全局左侧栏“设置”菜单项下方，作为进入工作台的入口，不再设置独立“工作台”主导航项；主区域依次展示工作台标题、terminal sessions 标识、当前项目上下文、依赖提示槽、会话 tabs 和终端面板；右侧检查器仅承载当前会话状态与项目文件夹交互，窄宽下排到首屏终端之后
- 添加项目文件夹：用户点击项目文件夹区右上角 `+` 后直接打开系统目录选择器；选择本机/已挂载目录后立即添加项目、进入该项目并加入最近项目列表
- 项目切换：左侧栏项目文件夹列表切换当前项目，中央会话列表和右侧项目文件夹按当前项目刷新；每个项目卡片右下角显示已打开 terminal window 数与 pane 总数，而不是固定“进入工作台”文案；旧项目的异步请求结果不得覆盖新项目 UI
...
- Prompt 优化浮层：终端工具栏提供 Prompt 优化入口，默认也可轻按 Control 唤起；浮层悬浮在当前终端输入光标附近，每次从关闭态重新打开时清空原始输入、中英文结果和状态提示；首次快捷键触发打开并聚焦原始 Prompt 输入框，再次触发自动优化并按设置语言填入当前 running active session；优化请求绑定当前项目根目录以加载项目 CLAUDE.md 上下文；优化失败时展示诊断信息，填入按钮仅在当前活动 session 为 running 且有优化结果时可用
