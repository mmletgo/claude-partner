...
**描述**：首页展示 GitHub 周热门项目，并可选使用本机 Claude CLI 生成中英文项目解说。

**功能点**：
- 抓取 GitHub Trending Weekly 全语言项目列表，展示仓库名称、简介、语言、星标、fork 与本周新增星标
- 首页项目卡片在桌面宽屏使用双列瀑布流布局，奇偶排名分列以形成错落卡片流并避免按行 grid 留空，排名与标题同行展示，卡片自身高度不强制等高；窄屏设备自动回落为单列并保持原始榜单顺序
- 对榜单按天缓存，频繁打开首页时优先使用缓存，GitHub 刷新失败时可回退最近缓存
