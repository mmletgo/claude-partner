...
- 终端窗口与 pane：每个工作台项目优先对应一个真实 tmux session，前端 tab 对应 tmux window，window 内分屏对应 tmux pane；运行期 attach 必须在连接项目 session 后切到对应 tmux window，确保 app tab 与 tmux window 一一绑定；没有 tmux、WSL 路径不可转换或后端不可用时退回单普通 PTY window。新建 window 在项目根目录启动系统 shell（用户自行运行 `claude` 或其他命令），支持 window tab 元数据持久化，重新打开 cc-partner 后应恢复之前打开的 window 列表
