...
- Worktree 管理：每个项目默认有一个主 worktree，路径为用户添加的项目根目录；顶部 worktree 管理层用于创建功能 worktree、切换 active worktree、查看 clean/dirty/conflict 状态，并提供 commit、push、merge 到主工作区和移除非主 worktree 操作。创建 worktree 时在应用数据目录下执行 `git worktree add -b <branch>`；commit 执行 `git add -A && git commit -m`；push 执行 `git push -u origin <branch>`；merge 前必须要求源 worktree 和主工作区均 clean 且无冲突；移除 worktree 前必须先关闭该 worktree 下的 terminal window。第一期不做 Git diff 面板、交互式冲突解决和 PR 创建
