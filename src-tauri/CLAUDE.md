...
├── workbench/         — 本机项目工作台：项目记录 + Git worktree + tmux 依赖管理 + 可恢复 PTY/tmux 终端会话 + 安全文件树 + 文件内容/预览 + 远端目录选择 helper [已实现]
...
- **远端目录选择 helper**：`workbench/remote_directory.rs` 只负责远端添加项目之前的目录浏览，不做项目根 containment、不注册 HTTP 路由。`remote_roots()` 返回存在的 home、Desktop、Documents、Downloads、home 下常用代码目录（`web_project/projects/workspace`）和平台根（Unix `/`、Windows 已存在盘符）；`list_remote_directory(path)` 读取一级目录，排序固定为目录优先、同类按小写名称再原始名称升序；`remote_path_info(path)` 返回 `dir/file`、readable、是否包含 `.git`、以及路径末段或路径字符串兜底的建议项目名。DTO 在 `workbench/models.rs`，字段 camelCase。
